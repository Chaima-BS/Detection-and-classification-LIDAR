...
UKF::UKF() {
    ...
    // initial state vector for CV
    x_cv_ = MatrixXd(5, 1);

    // initial state vector for CTRV
    x_ctrv_ = MatrixXd(5, 1);

    // initial state vector for RM
    x_rm_ = MatrixXd(5, 1);
...
void UKF::Cv(double p_x, double p_y, double v, double yaw, double yawd, double nu_a, double nu_yawdd,
  ...
  // Assessment for Driver Assistance Systems, 2016
    ...
    //predicted state values
    double px_p = p_x + v*sin(yaw)*delta_t;
    double py_p = p_y + v*cos(yaw)*delta_t;
